...
def iris_get_response(
    ...
    # Not entirely sure how the coeff array is organised, but the comment on the IDL version says,
    ...
    for j in range(0, 2):
        ...
        for k in range(0, ntt):
            ...
            # If you feel uneasy about this extrapolation, this is how iris_get_response.pro works implicitly
    ...
    if int(r["version"]) < 3:
        ...
        for j in range(0, sz[0]):
            ...
            for k in range(0, len(r["index_el_sji"][0])):
                pl_a = (
                    ...
                    * np.array(
                        [
                            [r["elements"][r["index_el_sji"][0]][i][1]]
                            for i in range(0, r["elements"][r["index_el_sji"][0]].shape[0])
                        ]
                    )
                ).T
        ...
        for nuv in range(0, 2):
            ...
            for k in range(0, len(r["index_el_sji"][nuv * 2])):
                arr = np.array(
                    [
                        [r["elements"][r["index_el_sji"][2:4, 3]][i]]
                        for i in range(0, r["elements"][r["index_el_sji"][2:4, 3]].shape[0])
                    ]
                )
...
def update_response_files(quiet=False):
    ...
    try:
        ...
        if not quiet:
            print(
                "Either hesperia is unreachable or you are not connected to the internet. Cannot check for new response files."
            )
    except:
        if not quiet:
