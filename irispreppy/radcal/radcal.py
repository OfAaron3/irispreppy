...
def radiometric_calibrate(ras, quiet=True, error=False):
    ...
    if response["NAME_SG"][0] == b"FUV" and response["NAME_SG"][1] == b"NUV":
        ...
        raise RuntimeError(
            "FUV and NUV cannot be found automatically. Please check ['NAME_SG'] from irisresponse above."
        )
    ...
    for key in indices:
        if key == "fdFUV":
            wvlns[key] = (
                np.arange(0, rasfits[indices[key]].header["NAXIS3"]) - rasfits[indices[key]].header["CRPIX3"] + 1
            ) * rasfits[indices[key]].header["CDELT3"] + rasfits[indices[key]].header["CRVAL3"]
            ...
            wvlns[key] = (
                np.arange(0, rasfits[indices[key]].header["NAXIS1"]) - rasfits[indices[key]].header["CRPIX1"] + 1
            ) * rasfits[indices[key]].header["CDELT1"] + rasfits[indices[key]].header["CRVAL1"]
            lamwin[key] = np.arange(0, len(wvlns[key]))[(wvlns[key] > FUV1[0]) & (wvlns[key] < FUV1[-1])]
            lamwin[key] = lamwin[key][0 : len(lamwin[key]) : len(lamwin[key]) - 1]
            rcfs[key] = weno4(wvlns[key][lamwin[key][0] : lamwin[key][1]], FUV1, eFUV1 / aFUV1) * const[key]
