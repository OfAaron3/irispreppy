...
def decon(rasfits, psfs, iterations=0, fft=False):
    ...
    for index, key in enumerate(indices):
        ...
        if type(iterations) == int:
            ...
            if len(iterations) != len(indices):
                ValueError(
                    "List or arrays of iterations must be same length as number of wavelength windows and in same order as in fits file."
                )
        ...
        for j in range(0, nlines):
            deconlst[index][j] = isd.IRIS_SG_deconvolve(
                rasfits[indices[key]].data[j], psf=psf2use, iterations=its, fft_div=fft
            )
        if not fft:
            deconlst[index][np.isnan(deconlst[index])] = 0

        hdr0["TDMEAN" + str(indices[key])] = np.mean(deconlst[index])
        hdr0["TDRMS" + str(indices[key])] = np.sqrt(
            np.sum((deconlst[index] - np.mean(deconlst[index])) ** 2) / deconlst[index].size
        )
    ...
    if fft:
        hdr0["HISTORY"] = "PSF deconvolution performed by division in Fourier space on " + dt.datetime.now().strftime(
            "%Y-%m-%d"
        )
    else:
        hdr0["HISTORY"] = "PSF deconvolution performed by Richardson-Lucy algorithm on " + dt.datetime.now().strftime(
            "%Y-%m-%d"
        )
...
def deconvolve(ras, iterations=0, fft=False):
    ...
    (See Courier et al. 2018 for more information - DOI: 10.1007/s11207-018-1347-9)
    ...
    if ras[0].header["HISTORY"][-1][:-11] == "FITS made with astropy on":
        ...
        if dt.strptime(ras[0].header["HISTORY"][-1][-10:], "%Y-%m-%d") < dt.strptime("2025-04-29", "%Y-%m-%d"):
            warnings.warn(
                "Future versions of deconvolve.py may behave unexpectedly with rasters calibrated before 2025-04-29. If you plan to deconvolve again, please calibrate again",
                UserWarning,
            )
